<?php
    ...
    // the post count of the post whose button was clicked
        ...
        $sql = "SELECT favorites FROM voting WHERE id = ? AND count = ? AND (upvotes = '1' OR downvotes = '1' OR favorites = '1')";
        ...
        if ($rows) {
            ...
            if ($rows[0][0] == '1') {
                apologize("You've already favorited this post!");
            } else {
                $sql = "UPDATE voting SET favorites = '1' WHERE id = ? AND count = ?";
            }      
        } else {
            $sql = "INSERT INTO voting (id, count, favorites) VALUES (?, ?, '1')";
        }
        ...
        $sql = "UPDATE submissions SET favorites = favorites + 1 WHERE count = ?";
        $stmt = mysqli_prepare($mysqli, $sql);
        ...
        if ($update === false)
        {
            apologize("Error: Could not record favorite.");
        }
        ...
        notify("You favorited :) but you're my favorite ;)");
    }
?>